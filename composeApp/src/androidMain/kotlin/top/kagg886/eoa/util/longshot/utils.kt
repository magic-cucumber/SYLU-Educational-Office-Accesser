...
internal class LazyListStateLongShotTarget(
    ...
    override fun canScrollVertically(direction: Int): Boolean {
        ...
        logger.i(
            "LazyListState target canScrollVertically(direction=$direction) -> $result，" +
                    "first=${lazyListState.firstVisibleItemIndex}，offset=${lazyListState.firstVisibleItemScrollOffset}",
        )
        return result
    }
    ...
    override fun scrollBy(x: Int, y: Int) {
        scrollY += y
        logger.i("LazyListState target scrollBy(x=$x, y=$y)，syntheticY=$scrollY")
        scope.launch {
            lazyListState.scrollBy(y.toFloat())
            logger.i(
                "LazyListState target scrollBy 完成，first=${lazyListState.firstVisibleItemIndex}，" +
    ...
    override fun getScrollY(): Int {
        logger.i("LazyListState target getScrollY -> $scrollY")
        return scrollY
    }
}
