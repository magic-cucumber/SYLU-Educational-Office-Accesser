 ...
 * ================================================
 * Author:     iveou
 * Created on: 2026/7/24 11:36
 * ================================================
 */

class DefaultAddableCookiesStorage(initial: List<Cookie> = listOf(), private val clock: () -> Long = { getTimeMillis() }) : CookiesStorage {
    private data class CookieWithTimestamp(val cookie: Cookie, val createdAt: Long)
    private val container: MutableList<CookieWithTimestamp> = initial.map { CookieWithTimestamp(it, clock()) }.toMutableList()
